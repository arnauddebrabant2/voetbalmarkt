    ...
    <div className="space-y-4">
      <div className="bg-gradient-to-br from-[#1E293B] to-[#0F172A] border border-white/10 rounded-2xl overflow-hidden">
        ...
        <div className="pt-12 px-6 pb-6">
          ...
          <Link href="/profiel" className="block w-full py-2.5 rounded-xl bg-[#F59E0B] hover:bg-[#D97706] text-white text-center font-semibold transition">
            Bekijk profiel
          </Link>
        </div>
      </div>
      ...
      <div className="bg-[#1E293B]/60 backdrop-blur-sm border border-white/10 rounded-2xl p-4">
        ...
        <div className="space-y-2">
          <Link href="/spelers" className="flex items-center gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl transition group">
            <span className="text-xl">👥</span>
            <span className="text-sm text-gray-300 group-hover:text-white">Zoek spelers</span>
          </Link>
          <Link href="/clubs" className="flex items-center gap-3 p-3 bg-white/5 hover:bg-white/10 rounded-xl transition group">
            <span className="text-xl">🏢</span>
...
function ClubsSidebar() {
  ...
  return (
    <div className="bg-[#1E293B]/60 backdrop-blur-sm border border-white/10 rounded-2xl p-6">
      ...
      {!playerProvince && (
        <p className="text-gray-400 text-sm">Vul je provincie in bij je profiel om clubs in je buurt te zien.</p>
      )}

      {playerProvince && clubsInBuurt.length === 0 && (
        <p className="text-gray-400 text-sm">Nog geen clubs gevonden in {playerProvince}.</p>
      )}
        ...
        <div className="space-y-3">
          {clubsInBuurt.map((club) => (
            ...
            >
              ...
              <p className="text-xs text-gray-500 mt-2 line-clamp-2">{club.bio || 'Geen beschrijving beschikbaar.'}</p>
            </Link>
          ))}
        </div>
      )}
...
export default function HomePage() {
  ...
  const [commentCounts, setCommentCounts] = useState<{ [key: string]: number }>({})  // ✅ FIX 2: eigen state voor comment counts
  ...
  // ✅ FIX 2: Haal comment counts op uit de comments tabel
  useEffect(() => {
    const fetchCommentCounts = async () => {
      ...
      if (data) {
        ...
        data.forEach(comment => {
          counts[comment.listing_id] = (counts[comment.listing_id] || 0) + 1
        })
  ...
  useEffect(() => {
    const fetchUserLikes = async () => {
      if (!user) return
      const { data } = await supabase.from('likes').select('listing_id').eq('user_id', user.id)
      if (data) setLikedPosts(new Set(data.map(like => like.listing_id)))
    }
    fetchUserLikes()
  }, [user])
  ...
  useEffect(() => {
    const fetchLikeCounts = async () => {
      ...
      const { data } = await supabase.from('likes').select('listing_id')
      if (data) {
        const counts: { [key: string]: number } = {}
        data.forEach(like => {
    ...
    if (listings.length > 0) fetchLikeCounts()
  }, [listings])
  ...
  const togglePosition = (pos: string) => {
    setForm((prev) => {
      ...
      return {
        ...prev,
        positions: exists ? prev.positions.filter((p) => p !== pos) : [...prev.positions, pos],
      }
    })
  }
  ...
  const toggleLike = async (listingId: string) => {
    ...
    if (isLiked) {
      ...
      const { error } = await supabase.from('likes').delete().eq('listing_id', listingId).eq('user_id', user.id)
      ...
      const { error } = await supabase.from('likes').insert({ listing_id: listingId, user_id: user.id })
  ...
  const openLikersModal = (listingId: string) => { setSelectedListingId(listingId); setShowLikersModal(true) }
  const closeLikersModal = () => { setShowLikersModal(false); setSelectedListingId(null) }

  const toggleComments = (listingId: string) => {
    const newExpanded = new Set(expandedComments)
    if (newExpanded.has(listingId)) newExpanded.delete(listingId)
    else newExpanded.add(listingId)
    setExpandedComments(newExpanded)
  }

  const getSortedListings = () => {
    if (sortOrder === 'popular') {
      return [...listings].sort((a, b) => (likeCounts[b.id] || 0) - (likeCounts[a.id] || 0))
    }
    return listings
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (form.positions.length === 0) { setMessage('❌ Selecteer minimaal 1 positie'); return }
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        ...
        ...(profile?.role === 'club'
          ...
          : { position: form.positions[0] || null, position_secondary: form.positions[1] || null }),
    ...
    if (!error && data) {
      ...
      setShowModal(false)
      setForm({ title: '', description: '', province: '', level: '', positions: [], available_from: '' })
  ...
  const allPositions = [
    'Doelman', 'Rechtsachter', 'Linksachter',
    'Centrale verdediger links', 'Centrale verdediger rechts',
    'Centrale middenvelder links', 'Centrale middenvelder rechts',
    'Aanvallende middenvelder', 'Linksbuiten', 'Rechtsbuiten', 'Spits',
  ]
      ...
      <main className="relative max-w-[1400px] mx-auto px-4 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr_300px] gap-6">
          ...
          <aside className="hidden lg:block">
            <div className="sticky top-24"><ProfileSidebar /></div>
          </aside>
            ...
            <div className="space-y-6">
              {getSortedListings().map((l, i) => {
                const pos = l.type === 'club_zoekt_speler' && l.positions_needed
                  ? l.positions_needed.split(',').map((p: string) => p.trim()).filter(Boolean)
                ...
                const timeAgo = diffDays <= 0 ? 'vandaag' : diffDays < 7 ? `${diffDays}d geleden` : created.toLocaleDateString('nl-BE')

                const displayName = l.profiles_player?.is_anonymous
                  ? l.profiles_player?.role === 'club' ? 'Anonieme club' : 'Anonieme speler'
                  : l.profiles_player?.display_name || 'Onbekend'

                // ✅ FIX 2: gebruik commentCounts state ipv l.comment_count
                ...
                return (
                  ...
                  >
                    ...
                    <div className="p-6">
                      <div className="flex items-start gap-4 mb-4">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className={`inline-flex items-center gap-1 text-xs font-semibold px-3 py-1 rounded-full ${
                            l.type === 'club_zoekt_speler' ? 'bg-[#F59E0B]/20 text-[#F59E0B]' : 'bg-green-600/20 text-green-400'
                          }`}>
                      ...
                      <div className="grid grid-cols-3 gap-3">
                        ...
                        <div className="bg-gradient-to-br from-[#F59E0B]/10 to-[#D97706]/10 border border-[#F59E0B]/20 rounded-xl p-3">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-[#F59E0B]">⚽</span>
                            <span className="text-xs text-[#F59E0B]/60">{pos.length > 1 ? 'Posities' : 'Positie'}</span>
                          </div>
                    ...
                    <div className="border-t border-white/5 px-6 py-3 flex items-center gap-4 text-sm text-gray-400">
                      ...
                      >
                        ...
                        <span className="flex items-center gap-1">
                          {likeCounts[l.id] > 0 ? (
                            <>
                              <span onClick={(e) => { e.stopPropagation(); openLikersModal(l.id) }} className="hover:underline font-semibold cursor-pointer">
                                {likeCounts[l.id]}
                              </span>
                      ...
                      >
                        ...
                        {/* ✅ FIX 2: commentCount ipv l.comment_count */}
                        <span>{commentCount > 0 ? `${commentCount} ${commentCount === 1 ? 'Reactie' : 'Reacties'}` : 'Reageer'}</span>
                      </button>
                    ...
                    {/* ✅ FIX 3: CommentsSection correct gebruiken - geen isOpen/onToggle props */}
                    <AnimatePresence>
          ...
          {/* Right Sidebar */}
          <aside className="hidden lg:block">
            <div className="sticky top-24"><ClubsSidebar /></div>
          </aside>
        </div>
      </main>
              ...
              <div className="relative bg-gradient-to-b from-[#1E293B] to-[#0F172A] border border-white/10 rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-gray-400 mt-1">
                        {profile?.role === 'club' ? 'Vertel welke spelers je zoekt voor je team' : 'Deel wat je zoekt met de community'}
                      </p>
                    </div>
                    <button onClick={() => setShowModal(false)} className="w-10 h-10 flex items-center justify-center rounded-xl bg-white/5 hover:bg-white/10 text-gray-400 hover:text-white transition">
                      <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <form onSubmit={handleSubmit} className="relative overflow-y-auto max-h-[calc(90vh-180px)]">
                  <div className="p-6 space-y-6">
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-300">✏️ Titel</label>
                      <input name="title" placeholder="Geef je bijdrage een pakkende titel..." value={form.title} onChange={handleChange} required
                    ...
                    <div className="space-y-2">
                      <label className="block text-sm font-medium text-gray-300">📝 Beschrijving</label>
                      <textarea name="description"
                        placeholder={profile?.role === 'club' ? 'Beschrijf welk type spelers je zoekt...' : 'Vertel over jezelf, je ervaring, wat je zoekt in een club...'}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-300">📍 Provincie</label>
                        <select name="province" value={form.province} onChange={handleChange} required
                          ...
                          {['Antwerpen','Limburg','Oost-Vlaanderen','West-Vlaanderen','Vlaams-Brabant','Brussel'].map(p => (
                            <option key={p} className="bg-[#1E293B]">{p}</option>
                          ))}
                        </select>
                      </div>
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-300">🏆 Niveau</label>
                        <select name="level" value={form.level} onChange={handleChange} required
                          ...
                          {['Recreatief / Vriendenploeg','4e Provinciale','3e Provinciale','2e Provinciale','1e Provinciale','3e Afdeling','2e Afdeling','1e Afdeling'].map(n => (
                            <option key={n} className="bg-[#1E293B]">{n}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      <div className="flex items-center justify-between">
                        <label className="block text-sm font-medium text-gray-300">
                          ⚽ {profile?.role === 'club' ? 'Welke posities zoek je?' : 'Op welke posities kan je spelen?'}
                        </label>
                        {form.positions.length > 0 && (
                          <span className="text-xs bg-[#F59E0B]/20 text-[#F59E0B] px-3 py-1 rounded-full">{form.positions.length} geselecteerd</span>
                        )}
                      </div>
                            ...
                            return (
                              <button key={pos} type="button" onClick={() => togglePosition(pos)}
                                className={`relative rounded-lg px-3 py-2.5 text-sm font-medium transition-all ${
                                  selected ? 'bg-gradient-to-r from-[#F59E0B] to-[#D97706] text-white' : 'bg-white/5 text-gray-300 hover:bg-white/10 border border-white/10'
                                }`}>
                                {selected && <span className="absolute top-1 right-1 w-2 h-2 bg-white rounded-full" />}
                                {pos}
                              </button>
                            )
                          })}
                    ...
                    {profile?.role === 'speler' && (
                      <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-300">📅 Beschikbaar vanaf (optioneel)</label>
                        <input type="date" name="available_from" value={form.available_from} onChange={handleChange}
                    ...
                    {message && (
                      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                        className={`p-4 rounded-xl flex items-center gap-3 ${
                          message.includes('✅') ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 'bg-red-500/10 text-red-400 border border-red-500/20'
                        }`}>
                  ...
                  <div className="sticky bottom-0 bg-gradient-to-t from-[#0F172A] via-[#0F172A] to-transparent border-t border-white/10 p-6">
                    <div className="flex items-center justify-between gap-4">
                      ...
                      <button type="submit" disabled={saving || form.positions.length === 0}
                        className="flex-1 max-w-xs px-6 py-3 rounded-xl bg-gradient-to-r from-[#F59E0B] to-[#D97706] text-white font-bold hover:shadow-xl hover:shadow-[#F59E0B]/50 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
