      ...
      <div className="w-full max-w-5xl backdrop-blur-xl bg-white/10 border border-white/20 rounded-3xl shadow-2xl text-white flex flex-col md:flex-row gap-8 md:gap-12 p-6 md:p-10">
        ...
        <div className="flex-1 flex flex-col justify-center">
          ...
          <p className="text-gray-300 text-sm md:text-base leading-relaxed max-w-md">
            {isRegister
              ? 'Registreer vandaag nog en maak een profiel aan zodat clubs je kunnen ontdekken.'
        ...
        <div className="flex-1">
            ...
            {isRegister && (
              <>
                ...
                <select
                  ...
                  value={role} onChange={(e) => setRole(e.target.value as 'speler' | 'trainer' | 'club')}>
                ...
                <div className="relative">
                  ...
                  <input type="text" placeholder="Weergavenaam (optioneel)"
                    className="w-full border border-white/30 bg-white/10 text-white placeholder-gray-300 p-3 pl-10 rounded-xl"
                    value={displayName} onChange={(e) => setDisplayName(e.target.value)} disabled={isAnonymous} />
                </div>
          ...
          {message && <p className="text-xs mt-4 text-center text-gray-300">{message}</p>}

          <div className="mt-6 text-center text-xs text-gray-300">
            {isRegister ? (
              <>Heb je al een account?{' '}
                <button onClick={() => setIsRegister(false)} className="text-[#F59E0B] underline hover:text-[#D97706]">Inloggen</button>
              </>
            ) : (
              <>Nog geen account?{' '}
                <button onClick={() => setIsRegister(true)} className="text-[#F59E0B] underline hover:text-[#D97706]">Registreren</button>
              </>
            )}
          </div>
        </div>
...
// ✅ FIX: Suspense wrapper als default export
